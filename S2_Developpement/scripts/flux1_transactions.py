...
def main(date_traitement: str):
    ...
    df_converti = (
        ...
        .withColumn(
            ...
            .when(col("taux_conversion").isNotNull(), col("montant") / col("taux_conversion"))
            .otherwise(lit(None).cast("double"))
        )
        .drop("taux_conversion")
    )

    nb_non_convertis = df_converti.filter(
        col("montant_eur").isNull() & (col("devise") != "EUR")
    ).count()
    if nb_non_convertis > 0:
        print(f"[Flux1] WARN : {nb_non_convertis} transactions avec devise absente du référentiel — montant_eur NULL (exclues des agrégations)")
